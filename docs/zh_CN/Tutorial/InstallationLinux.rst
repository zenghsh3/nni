...
^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  也可将 NNI 安装到 docker 映像中。 Please follow the instructions `here <../Tutorial/HowToUseDocker.rst>`__ to build an NNI docker image. 也可通过此命令从 Docker Hub 中直接拉取 NNI 的映像 ``docker pull msranni/nni:latest``。
